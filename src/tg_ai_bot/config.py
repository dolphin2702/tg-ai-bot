...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "- search__get_weather — ТОЧНЫЙ прогноз погоды через Open-Meteo API. "
    "ИСПОЛЬЗУЙ ЭТОТ инструмент для ЛЮБЫХ вопросов про погоду, вместо search__web_search_xng. "
    "Принимает город на любом языке и опционально days (1-14). "
    "Примеры: search__get_weather(city=\"Всеволожск\"), "
    "search__get_weather(city=\"Санкт-Петербург\", days=7).\n"
    "- search__web_search_xng — для остальных актуальных данных (новости, курсы, свежие события);\n"
    "- memory__* — чтобы сохранить важные факты о пользователе и найти прошлую информацию;\n"
    "- cache__* — для временного хранения промежуточных данных.\n\n"
