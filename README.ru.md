...
### Как бот отвечает в группе

Бот реагирует на сообщение в группе **только если**:

1. Есть упоминание `@your_bot_username`.
2. Сообщение — ответ (reply) на сообщение бота.
3. Сообщение начинается с одного из слов в `GROUP_TRIGGERS` (по умолчанию: `ии,ai,бот,помощник`).

Примеры (при `GROUP_TRIGGERS=ии,ai,бот,помощник`):

| Сообщение в группе | Реакция |
|---|---|
| `ИИ, какая погода в Самаре?` | ✅ отвечает на «какая погода в Самаре?» |
| `Бот, помоги с рецептом` | ✅ отвечает на «помоги с рецептом» |
| `@your_bot привет` | ✅ отвечает на «привет» |
| (reply боту) `а подробнее?` | ✅ отвечает |
| `привет всем` | ❌ молчит |
| `как дела` | ❌ молчит |

Префикс-слово — настраивается через `GROUP_TRIGGERS`, регистр не важен.
Разделитель после слова обязателен (`ИИ,`, `ИИ `, `ИИ:` — сработает; `ИИИ` — нет).

### Автодоступ из групп

Любой, кто хотя бы раз написал в авторизованной группе (из `ALLOWED_CHATS`),
автоматически получает доступ к боту в личке. Ручного добавления в
`ALLOWED_USERS` не требуется. Список известных пользователей хранится в Redis.
